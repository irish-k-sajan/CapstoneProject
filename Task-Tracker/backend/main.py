...
async def create_users(user_id:str,emp:EmployeeBase,db: db_dependency):
    user_exists=db.query(models.Employee).filter(models.Employee.employee_id==user_id).first()
    if user_exists:
...
def delete_user_role(project_id: str,user_id: str, db:db_dependency):
    admin=check_admin_id(user_id,db)
    if admin:   
        db_user_roles=db.query(models.UserRole).filter(models.UserRole.project_id==project_id).all()
        print(db_user_roles)
